...
// SAMPLER FOR alpha IF NEEDED
/*
// [[Rcpp::export]]
double sample_alpha(const double& alpha, const double& a_alpha, const double& b_alpha, const double& K, const double& I) 
{
     // Step 1: Sample auxiliary variable eta ~ Beta(alpha + 1, I)
     double eta = R::rbeta(alpha + 1.0, I);
     
     // Step 2: Compute mixture weight
     double pi = (a_alpha + K - 1.0) / (I * (b_alpha - log(eta)) + a_alpha + K - 1.0);
     
     // Step 3: Sample new alpha from a mixture of two Gamma distributions
     double new_alpha;
     if (R::runif(0, 1) < pi) {
          new_alpha = R::rgamma(a_alpha + K, 1.0 / (b_alpha - log(eta)));
     } else {
          new_alpha = R::rgamma(a_alpha + K - 1, 1.0 / (b_alpha - log(eta)));
     }
...
// [[Rcpp::export]]
List sample_Xi(const double& I, const double& alpha, const double& mu, const double& sigsq, 
               vec Lambda, uvec Xi, const vec& Y) {
     ...
     for (uword i = 0; i < I; i++) {
          ...
          for (uword k = 0; k < K; k++) {
               logprobs[k] = log(cluster_sizes[k]) - log(I - 1.0 + alpha);
          ...
          logprobs[K] = log(alpha) - log(I - 1.0 + alpha);
...
// [[Rcpp::export]]
List WAIC(const double& I, const double& B, const vec& Y, const List& Lambda_chain, const umat& Xi_chain) {
