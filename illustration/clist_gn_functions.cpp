...
vec get_Eta(const double& K, const double& zeta, const mat& U) 
{
     ...
     // Compute eta_{k,l} using latent positions
     for (uword k = 0; k < K; k++) {
          for (uword l = k; l < K; l++) {
               eta_index = get_k_diag(k, l, K);
               out[eta_index] = zeta - norm(U.row(k) - U.row(l), 2);
          }
     }
...
// [[Rcpp::export]]
List sample_Xi(const double& I, const double& Q, const double& gamma, const double& zeta, 
     ...
     for (uword i = 0; i < I; i++) {
          ...
          for (uword k = 0; k < K; k++) {
               ...
               for (uword j = 0; j < I; j++) {
                    ...
                    eta_kj = zeta - norm(U.row(k) - U.row(Xi[j]), 2);
          ...
          for (uword j = 0; j < I; j++) {
               ...
               y_index = (i < j) ? get_k(i, j, I) : get_k(j, i, I);
               eta_new = zeta - norm(new_U.row(K) - new_U.row(Xi[j]), 2);
