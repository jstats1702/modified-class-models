...
// [[Rcpp::export]]
vec sample_Y(const double& I, const vec& Eta, const uvec& Xi, const vec& na_indices, vec Yna) {
     // Sample NA values in Y
     uword k, eta_index, K = arma::max(Xi) + 1;
     ...
     for (uword i = 0; i < I - 1; i++) {
          for (uword ii = i + 1; ii < I; ii++) {
               ...
               if (static_cast<bool>(na_indices[k])) {
                    eta_index = get_k_diag(std::min(Xi[i], Xi[ii]), std::max(Xi[i], Xi[ii]), K);
                    Yna[k] = R::rbinom(1, R::pnorm(Eta[eta_index], 0.0, 1.0, 1, 0));
               }
          }
     }
     return Yna;
