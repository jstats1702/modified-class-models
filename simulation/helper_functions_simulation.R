...
get_results_dm <- function(base_path, model, prior, density_type, cluster_type, size, n_thin){
     require(mclust)
     ...
     # Load data
     load(paste0(base_path, "simulation_", model, "_", prior, "_", dataset, ".RData"))
     ...
     Ycube <- sbm_simulation$adjacency_matrix
     labels_true <- sbm_simulation$communities
     ...
     # Compute posterior mean of the VI distance from the true partition to MCMC Xi samples
     vi_distance <- VI(labels_true, samples$Xi_chain)

     # Compute quartiles of nonempty clusters
     H_quartiles <- quantile(post_nclusters)

     # Compute VI distance from the true partition to point estimate
     vi_point_estimate <- VI(cls = labels_true, cls.draw = t(as.matrix(cluster_assignments)))

     # Compute horizontal bound of the credible ball
     cb <- credibleball(c.star = cluster_assignments, cls.draw = samples$Xi_chain)
     horizontal_bound_credible_ball <- cb[[5]]
     
     # Compute posterior mean False Discovery Rate (FDR) and False Negative Rate (FNR)
     posterior_fdr_fnr <- compute_posterior_fdr_fnr(samples$Xi_chain, sbm_simulation$communities)

     # Compute FDR and FNR for point estimate
     fdr_fnr_point <- compute_fdr_fnr(labels_true, cluster_assignments)
     
     # Preallocate vector to store ARIs
     ari_vec <- numeric(B)
     for (b in seq_len(B)) {
          ari_vec[b] <- adjustedRandIndex(samples$Xi_chain[b, ], labels_true)
     }
     ...
     list(
          ...
          H_quartiles = H_quartiles,
          cluster_assignments = cluster_assignments,
          vi_point_estimate = vi_point_estimate,
          horizontal_bound_credible_ball = horizontal_bound_credible_ball,
          posterior_fdr_fnr = posterior_fdr_fnr,
          labels_true = labels_true,
          ari_vec = ari_vec
     )
}

get_results_np <- function(base_path, model, prior, density_type, cluster_type, size, n_thin){
     require(mclust)
