...
base_path <- "C:/Users/User/Dropbox/PAPERS/projects/eleni/drug/"
setwd(base_path)
...
results_drug_class_dm <- get_results_dm(base_path,
                                        model   = "class", 
                                        prior   = "dm",
                                        dataset,
                                        n_thin)

results_drug_class_dp <- get_results_np(base_path,
                                        model   = "class",
                                        prior   = "dp",
                                        dataset,
                                        n_thin)

results_drug_class_py <- get_results_np(base_path,
                                        model   = "class",
                                        prior   = "py",
                                        dataset,
                                        n_thin)

results_drug_class_gn <- get_results_np(base_path,
                                        model   = "class",
                                        prior   = "gn",
                                        dataset,
                                        n_thin)
...
results_drug_clist_dm <- get_results_dm(base_path,
                                        model   = "clist",
                                        prior   = "dm",
                                        dataset,
                                        n_thin)

results_drug_clist_dp <- get_results_np(base_path,
                                        model   = "clist",
                                        prior   = "dp",
                                        dataset,
                                        n_thin)

results_drug_clist_py <- get_results_np(base_path,
                                        model   = "clist",
                                        prior   = "py",
                                        dataset,
                                        n_thin)

results_drug_clist_gn <- get_results_np(base_path,
                                        model   = "clist",
                                        prior   = "gn",
                                        dataset,
                                        n_thin)
...
results_drug_cleigen_dm <- get_results_dm(base_path,
                                          model   = "cleigen",
                                          prior   = "dm",
                                          dataset,
                                          n_thin)

results_drug_cleigen_dp <- get_results_np(base_path,
                                          model   = "cleigen",
                                          prior   = "dp",
                                          dataset,
                                          n_thin)

results_drug_cleigen_py <- get_results_np(base_path,
                                          model   = "cleigen",
                                          prior   = "py",
                                          dataset,
                                          n_thin)

results_drug_cleigen_gn <- get_results_np(base_path,
                                          model   = "cleigen",
                                          prior   = "gn",
                                          dataset,
                                          n_thin)

# save(results_drug_class_dm,
#      results_drug_class_dp,
#      results_drug_class_py,
#      results_drug_class_gn,
#      results_drug_clist_dm,
#      results_drug_clist_dp,
#      results_drug_clist_py,
#      results_drug_clist_gn,
#      results_drug_cleigen_dm,
#      results_drug_cleigen_dp,
#      results_drug_cleigen_py,
#      results_drug_cleigen_gn,
#      file = "results_drug.RData")

load("C:/Users/User/Dropbox/PAPERS/projects/eleni/drug/results_drug.RData")
...
labels_list <- lapply(names_list, function(name) {
     eval(parse(text = paste0("results_drug_", name, "$sorted_network_data$labels")))
})
...
cluster_summary_df <- do.call(rbind, lapply(names_list, function(name) {
     samples <- eval(parse(text = paste0("results_drug_", name, "$post_nclusters")))
...
cluster_samples <- lapply(names_list, function(name) {
     eval(parse(text = paste0("results_drug_", name, "$post_nclusters")))
})
...
for (name in names_list) {
     test_post <- eval(parse(text = paste0("results_drug_", name, "$test_stats$test_stats")))
     test_obs  <- eval(parse(text = paste0("results_drug_", name, "$test_stats$test_obs")))
...
for (stat in test_names) {
     ...
     ggsave(filename = paste0("plot_test_stats_", gsub(" ", "_", stat), "_drug.pdf"),
...
for (name in names_list) {
     ...
     perf_stats <- eval(parse(text = paste0("results_drug_", name, "$pred_performance_stats")))
...
for (model in models) {
     for (prior in priors) {
          obj_name <- paste0("results_drug_", model, "_", prior)
          tmp <- data.frame(
               Model    = toupper(model),
               Prior    = toupper(prior),
